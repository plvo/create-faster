---
title: Introduction
description: Modern, type-safe CLI scaffolding tool for production-ready projects
---
...
The defining feature of create-faster is its ability to scaffold multiple applications in a single command. Need a Next.js frontend, an Expo mobile app, and a Hono backend? Create all three at once with automatic Turborepo configuration.

**Single app** projects use the framework's standard structure:

```bash
bunx create-faster my-app
# Creates a single Next.js/Expo/Hono application
```

**Multi-app** projects automatically configure Turborepo:

```bash
bunx create-faster my-saas
# Prompted to create multiple apps → automatic monorepo setup
```
...
- Module configuration (shadcn/ui, Better Auth, TanStack Query, etc.)
...
- [Understand the project structure](/docs/structure) for single and monorepo setups
