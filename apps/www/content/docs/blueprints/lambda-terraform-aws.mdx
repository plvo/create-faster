...
description: AWS Lambda monorepo with API Gateway, SQS worker, and EventBridge cron
---

## Presentation

Serverless monorepo for AWS Lambda using Terraform as the infrastructure framework. Same three-function architecture as the SST variant (API Gateway, SQS, EventBridge), but with modular Terraform configurations, S3 remote state, and a `justfile` for local operations. Includes GitHub Actions CI/CD with OIDC authentication.
